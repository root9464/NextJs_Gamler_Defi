...
export const GameField = () => {
  ...
  return (
    <motion.div className='relative h-full w-full' ref={constraintsRef}>
      <Image src={Curash} alt='ntf' className='h-auto w-full object-contain' />
      <motion.div
        drag
        dragConstraints={constraintsRef}
        dragElastic={0}
        dragMomentum={false}
        className='rounded-6 absolute top-6 left-6 flex h-12 w-fit cursor-pointer items-center gap-2 border border-[#9D16B8] bg-white px-3 py-2'>
        <RocketIco />
