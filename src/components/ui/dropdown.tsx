...
const dropdownItemStyles = tv({
  ...
  variants: {
    ...
    isHovered: {
      ...
      false: ['bg-transparent text-fg', '**:data-[slot=icon]:text-muted-fg', '[&_.text-muted-fg]:text-muted-fg'],
    },
  },
});
...
const dropdownSectionStyles = tv({
  slots: {
    ...
    header: 'col-span-full px-3.5 py-2 font-medium text-muted-fg text-sm/6 sm:px-3 sm:py-1.5 sm:text-xs/5',
  },
});
...
const DropdownItem = ({ className, children, ...props }: DropdownItemProps) => {
  ...
  return (
    <ListBoxItemPrimitive
      textValue={textValue}
      className={composeRenderProps(className, (className, renderProps) => dropdownItemStyles({ ...renderProps, className }))}
      {...props}>
...
const DropdownDescription = ({ className, ref, ...props }: DropdownDescriptionProps) => (
  <Text slot='description' ref={ref} className={twMerge('text-muted-fg col-start-2 text-sm', className)} {...props} />
);

const DropdownSeparator = ({ className, ...props }: SeparatorProps) => (
  <Separator orientation='horizontal' className={twMerge('bg-fg/10 col-span-full -mx-1 my-1 h-px', className)} {...props} />
);
...
const DropdownKeyboard = ({ className, ...props }: React.ComponentProps<typeof Keyboard>) => {
  return (
    <Keyboard
      classNames={{
        base: twMerge('absolute right-2 pl-2 group-hover:text-primary-fg group-focus:text-primary-fg', className),
      }}
      {...props}
    />
  );
...
export type { DropdownDescriptionProps, DropdownItemProps, DropdownLabelProps, DropdownSectionProps };
