...
import { DropdownDescription, dropdownItemStyles, DropdownKeyboard, DropdownLabel, dropdownSectionStyles, DropdownSeparator } from './dropdown';
...
const MenuTrigger = ({ className, ref, ...props }: MenuTriggerProps) => (
  <Button
    ...
    className={composeTailwindRenderProps(className, 'focus-visible:ring-primary relative inline text-left outline-hidden focus-visible:ring-1')}
    {...props}
  />
);
...
const MenuContent = <T extends object>({ className, placement, popover, ...props }: MenuContentProps<T>) => {
  return (
    <PopoverContent className={composeTailwindRenderProps(popover?.className, 'min-w-40')} placement={placement} {...popover}>
      <MenuPrimitive
        ...
        <>
          {values.isSelected && (
            <>
              ...
              {values.selectionMode === 'multiple' && <IconCheck className='-mx-0.5 mr-2 size-4' data-slot='check-indicator' />}
            </>
          )}
